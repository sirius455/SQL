...
/*
    ...
    # VARCHAR2 (n)
      ...
      - VARCHAT2 (10)으로 설정된 컬럼의 데이터로 2글자가 들어오면 2바이트만 사용한다 
    ...
    # CHAR
     ...
     - 데이터가 낭비될 수는 있지만 불필요한 크기 계산이 없기 때문에 더 빠르다
...
-- 연습1 : 모든 사원의 사원번호/이름/월급/부서번호만를 조회해보세요 
...
-- MVL(컬럼명, 대체값) :  해당 컬럼에서 NULL값이 나왔을 때 대체값을 사용한다
...
-- SELECT DISTINCT : 중복되는 내용이 여러개 있어도 한번만 출력된다
